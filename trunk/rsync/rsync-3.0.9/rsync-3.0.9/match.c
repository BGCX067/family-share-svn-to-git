...
static void build_hash_table(struct sum_struct *s)
{
	...
	//动态计算哈希表的大小，使哈希表能够容纳80%的大文件，一个比传统大小大的数必须是技术，否则s2将不能跨越整个集合
...
//传输数据或者匹配标记
//i>0表示匹配的数量，i<0表示只有数据，-1表示还会送一个0int型标记，
//-2表示只送数据
			...
			struct map_struct *buf, OFF_T len)
{
	...
	//want_i变量用来支持邻近匹配，允许RLL输出代码使运行更有效率
	...
	do {
		...
		do {
			...
			//当更新原位时，最可能的匹配是用一个完全相同的偏移量，所以我们
			//希望在邻近的want_i最佳化
...
check_want_i:
			//我们已经找到了一个匹配，但现在检验want_i是否能够示意一个更好的匹配
	  ...
	  null_hash:
		...
		//通过早早匹配，我们避免重复读取数据三次当一个标记
		//自从上次匹配后走了很长一段路过来
		//这3次读取由运行时的匹配，校验和更新和数据发送引起
...
//浏览一个原文件，从生成器中寻找匹配校验和的块，然后传输数据或标记
//同时也用md计算器计算整个文件的md4校验和，这和文件一起传输，为了避免线路的故障
//参数：s，是来自生成器的校验和，如果<tt>s->count ==0</tt>，那么这个文件没有
//校验和
//参数：len，是将要传输文件的长度
/**
