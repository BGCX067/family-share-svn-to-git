...
//删除一个文件或目录。如果DEL_RECURSE标志设置，将会递归删除
//注意到fbuf必须指向一个最大路径长度的缓冲区如果指向的是一个目录（缓冲区用来递归，但是返回时不改变）
...
//设置（初始化）在 per-file sum_struct的项目和大小
//计算动态的块和校验和大小
//这只在generate_and_send_sums（）中调用，但是是一个独立的模块，封装了逻辑
//这个块大小大约是文件大小的平方根
//校验和大小决定根据是：
...
//生成并且传送一个签名/校验和数据流，描述一个缓冲区
//大约每block_len个字节生产一个校验和
/*
...
//这个方法只为普通文件调用。我们返回-2如果处理文件完成，返回-1如果没有关联目的地
//或者一个非负值如果我们找到一个交替的基本的文件
...
//这只为非常规文件调用。我们返回-2如果我们完成了处理，-1如果没有相关目标，或非负如果发现交替的基本文件
...
//在指定的文件列表的名字为fname的项目上活动，如果是目录，确保存在
//并且有正确的允许或时间戳信息。对于其它非常规文件（symlinks，等）我们创建它们。
//对于常规的已经变化的文件，我们尝试找到基本的分解并且开始传送校验和。
//ndx是文件的唯一索引值
//fname参数必须指向一个最大路径长度的缓冲区！（它经过delete_item()，我们可以用它来递归删除）
//注意到：当做最终目录允许并且修改时间修复时f_out设置成-1
